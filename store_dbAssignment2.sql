...
-- 4. Price greater than $50
SELECT * FROM products WHERE price > 50;

-- 5. Electronics category
SELECT * FROM products WHERE category = 'Electronics';

-- 6. Rating 4.5 or higher
SELECT * FROM products WHERE rating >= 4.5;

-- 7. Clothing AND price less than $30
SELECT * FROM products WHERE category = 'Clothing' AND price < 30;

-- 8. Home category OR stock quantity greater than 100
SELECT * FROM products WHERE category = 'Home' OR stock_quantity > 100;

-- 9. Available AND NOT in Toys
SELECT * FROM products WHERE is_available = TRUE AND category != 'Toys';

-- 10. Price between $20 and $100
SELECT * FROM products WHERE price BETWEEN 20 AND 100;

-- 11. Sorted by price ascending
SELECT * FROM products ORDER BY price ASC;

-- 12. Sorted alphabetically by name
SELECT * FROM products ORDER BY name ASC;

-- 13. Sorted by rating descending, then price ascending
SELECT * FROM products ORDER BY rating DESC, price ASC;

-- 14. 5 most expensive products
SELECT * FROM products ORDER BY price DESC LIMIT 5;

-- 15. 10 highest stock quantity
SELECT * FROM products ORDER BY stock_quantity DESC LIMIT 10;

-- 16. Alphabetically, results 6–10
SELECT * FROM products ORDER BY name ASC LIMIT 5 OFFSET 5;

-- 17. Name contains "Premium"
SELECT * FROM products WHERE name LIKE '%Premium%';

-- 18. Name starts with "S"
SELECT * FROM products WHERE name LIKE 'S%';

-- 19. Name ends with "er"
SELECT * FROM products WHERE name LIKE '%er';

-- 20. Description contains "with"
SELECT * FROM products WHERE description LIKE '%with%';

-- 21. Category contains "on" (case-insensitive)
SELECT * FROM products WHERE category ILIKE '%on%';

-- 22. Name is exactly 5 characters long
SELECT * FROM products WHERE LENGTH(name) = 5;

-- 23. Released in 2023
SELECT * FROM products WHERE EXTRACT(YEAR FROM release_date) = 2023;

-- 24. Products with no rating
SELECT * FROM products WHERE rating IS NULL;

-- 25. 3 most recently released available products
SELECT * FROM products WHERE is_available = TRUE ORDER BY release_date DESC LIMIT 3;

-- 26. Books or Stationery with price less than $25
SELECT * FROM products WHERE category IN ('Books', 'Stationery') AND price < 25;

-- 27. Product names alongside their lengths
SELECT name, LENGTH(name) AS name_length FROM products;

-- 28. Product names in uppercase
SELECT UPPER(name) AS name_uppercase FROM products;

-- 29. First 10 characters of description
SELECT name, SUBSTRING(description, 1, 10) FROM products;

-- 30. Replace "Premium" with "Luxury" in names
SELECT REPLACE(name, 'Premium', 'Luxury')  FROM products;

-- 31. Concatenate name and category with hyphen
SELECT name || ' - ' || category AS name_category FROM products;

-- 32. Round prices to nearest integer
SELECT name, ROUND(price) AS rounded_price FROM products;

-- 33. Sale price after 15% discount
SELECT name, ROUND(price * 0.85, 2)  FROM products;

-- 34. Day of the week each product was released
SELECT name, TO_CHAR(release_date, 'Day') AS release_day FROM products;

-- 35. Products released in the current month (any year)
SELECT * FROM products WHERE EXTRACT(MONTH FROM release_date) = 
EXTRACT(MONTH FROM CURRENT_DATE);

-- 36. Days in inventory since release date
SELECT name, CURRENT_DATE  FROM products;

-- 37. Average price of all products
SELECT AVG(price) FROM products;

SELECT ROUND(AVG(price), 2) FROM products;

-- 38. Total stock quantity
SELECT SUM(stock_quantity) FROM products;

-- 39. Highest and lowest ratings
SELECT MAX(rating) , MIN(rating)  FROM products;

-- 40. Count of available products
